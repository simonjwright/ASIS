...
package A4G.Mapping is
   ...
   --  transfer the parent node from Node_To_Element_New function to
   --  Asis_Internal_Element_Kind function.
   --
   --  This variable is defined in the package spec, because it is needed
   --  by the current implementation of Enclosing_Element and by the calls to
   --  Asis_Internal_Element_Kind or to N_..._Mapping functions outside
   --  Node_To_Element_New function. Note, that in all cases the caller is
   --  responsible for proper svalue of this global variable.
   ...
   --  A4G.Decl_Sem.Search_First_View
