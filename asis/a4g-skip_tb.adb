...
package body A4G.Skip_TB is
   ...
   --  will be raised
   --  Note the special case of a GNAT-SPARK-specific 'Update attribute - it
   --  has non-standard syntax structire, see GNAT RM for details.
...
--  New Ada 2012 attributes. To be alphabetically ordered later
      A_Max_Alignment_For_Allocation_Attribute => No_Brackets'Access,
      An_Overlaps_Storage_Attribute => No_Brackets'Access,
--  |A2012 end
      ...
      A_Parenthesized_Expression       => Search_Right_Parenthesis'Access,
      A_Raise_Expression               => No_Brackets'Access,
   ...
   is
   begin
      if Is_Nil (Attribute_Designator_Expressions (E))
        or else
         (Int_Kind (E) = An_Implementation_Defined_Attribute
         and then
          To_Lower (To_String (
            Name_Image (Attribute_Designator_Identifier (E)))) = "update")
      then
         return No_Brackets (E, S);
      else
         return Search_Right_Parenthesis (E, S);
   ...
   begin
      ...
      if Get_Character (S) = ';' then
         ...
         --  pathological case of empty definition with private part
         S_P := S + 7;
      else
         S_P := S;
