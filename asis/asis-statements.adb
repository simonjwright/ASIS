...
package body Asis.Statements is
   ...
   begin
      ...
      if Arg_Kind not in A_Raise_Statement | A_Raise_Expression then
   ...
   begin
      ...
      while not (Nkind (Result_Node) = N_Object_Declaration
                 and then
                  Comes_From_Source (Result_Node))
      loop
...
------------------------------------------------------------------------------
      ...
      then
         ...
         if No (Parameter_Associations (Arg_Node))
          or else
            ...
            and then
             not (Is_Call_Through_Access_To_Subprogram (Statement)
                or else
                  Is_Dispatching_Call (Statement)))
         then
            return Nil_Element_List;
         else
   ...
   function Case_Path_Alternative_Choices
     (Path : Asis.Path)
   ...
   begin
      ...
      Check_Validity
        (Path, Package_Name & "Case_Path_Alternative_Choices");

      if Arg_Kind not in A_Case_Path | A_Case_Expression_Path then
         Raise_ASIS_Inappropriate_Element
           (Package_Name & "Case_Path_Alternative_Choices",
            Wrong_Kind => Arg_Kind);
      end if;
   ...
   exception
      ...
      when Ex : others =>
         Report_ASIS_Bug
           (Query_Name  => Package_Name & "Case_Path_Alternative_Choices",
            Ex          => Ex,
            Arg_Element => Path);
   end Case_Path_Alternative_Choices;
