...
package body Asis.Data_Decomposition.Set_Get is
   ...
   procedure Set_Named_Components (E : Element; List_Kind : List_Kinds) is
      ...
      begin
         ...
         --  Append only if it's a defining identifier and is a component.
         --  For example:
         --     record
         --        X : access procedure (Param : ...);
         --  we want to append X, but not Param (Param is not a component).

         if Int_Kind (Element) /= A_Defining_Identifier
           or else not Ekind_In (Node (Element), E_Component, E_Discriminant)
         then
            return;
         end if;
