...
--  --|A2012 end
      ...
      --  The following are for gnat2xml. It treats a Compilation_Unit like an
      --  Element. It treats the various information encoded in Trait_Kinds as
      --  elements. It treats Subprogram_Default_Kinds as Elements, whereas
      --  ASIS only does so for A_Name_Default. It needs an extra Kind for
      --  lists of elements.

      A_Comment,
      --  Comments do not appear in the ASIS tree as Elements; they are fetched
      --  from the original source file as necessary. However, generated
      --  Ada_Trees can have Kind = A_Comment.

      --  The following are for "traits". In the XML output, if the "aliased"
      --  keyword is present, we have An_Aliased element, otherwise
      --  Not_An_Element, and similarly for "not null", "reverse", "limited",
      --  and "abstract". ???Perhaps we should treat modes in a similar manner.

      An_Aliased,
      A_Null_Exclusion, -- "not null"
      A_Not_Null_Return, -- null exclusion on function result
      A_Reverse,
      A_Limited,
      A_Synchronized,
      A_Private,
      ...
      --  The following represent lists.  These mostly correspond to the _List
      --  subtypes declared in Asis, "3.8 subtypes of Element and
      --  Element_List", with an added "A_" or "An_" prefix. See gnat2xml for
      --  more details.

      --  The ones marked "not used" below correspond to subtypes declared in
      --  Asis, but never used as the return type of a query.
      ...
      An_Expression_List,
      --  A_Generic_Formal_Parameter_List, -- not used
      --  An_Identifier_List, -- not used
      A_Name_List,
      A_Parameter_Specification_List,
      A_Path_List,
      --  A_Pragma_Element_List, -- not used
      A_Record_Component_List,
      --  A_Representation_Clause_List, -- not used
      A_Statement_List,
      --  A_Variant_Component_List, -- not used, and it's not clear what it
      --  means. It can't be a list of Variant_Components, because there's no
      --  such thing.
      A_Variant_List,

      --  The following represent "abstract classes", i.e. sets of Kinds.
      --  These mostly correspond to the subtypes declared in Asis,
      --  "3.8   subtypes of Element and Element_List", with an added "A_" or
      --  "An_" prefix, and a "_Class" suffix. See gnat2xml for more details.

      --  The ones marked "needed for list" below correspond to subtypes
      --  declared in Asis, that are never used as the return type of a query,
      --  but the corresponding _List type is used.

      --  The ones marked "singleton, not needed" correspond to subtypes
      --  declared in Asis, that have a single Kind, so we don't need an
      --  abstract class.

      An_Element_Class,
      --  An_Access_Type_Definition_Class, -- not used
      An_Association_Class,
      --  A_Case_Statement_Alternative_Class, -- not used
      --  A_Clause_Class, -- not used
      --  A_Component_Clause_Class, -- singleton, not needed
      --  A_Component_Declaration_Class, -- not used
      --  A_Component_Definition_Class, -- singleton, not needed
      A_Constraint_Class,
      A_Context_Clause_Class, -- needed for list
      A_Declaration_Class,
      A_Declarative_Item_Class, -- needed for list
      A_Definition_Class,
      A_Discrete_Range_Class,
      A_Discrete_Subtype_Definition_Class,
      --  A_Discriminant_Association_Class, -- singleton, not needed
      A_Defining_Name_Class,
      --  An_Exception_Handler_Class, -- singleton, not needed
      An_Expression_Class,
      --  A_Formal_Type_Definition_Class, -- not used
      --  A_Generic_Formal_Parameter_Class, -- not used
      --  An_Identifier_Class, -- not used
      A_Name_Class,
      --  A_Parameter_Specification_Class, -- singleton, not needed
      A_Path_Class, -- needed for list
      A_Pragma_Element_Class,
      A_Range_Constraint_Class,
      A_Record_Component_Class, -- needed for list
      --  A_Record_Definition_Class, -- not used
      --  A_Representation_Clause_Class, -- not used
      --  A_Root_Type_Definition_Class, -- not used
      --  A_Select_Alternative_Class, -- not used
      A_Statement_Class, -- needed for list
      --  A_Subtype_Indication_Class, -- singleton, not needed
      --  A_Subtype_Mark_Class, -- not used
      --  A_Type_Definition_Class, -- not used
      --  A_Variant_Class, -- singleton, not needed
...
------------------------------------------------------------------------------
   ...
   subtype Internal_Expression_Kinds is Internal_Element_Kinds
      range A_Box_Expression .. A_For_Some_Quantified_Expression;
...
------------------------------------------------------------------------------
   ...
   pragma Ordered (Internal_Element_Kinds);
   --  Internal_Element_Kinds is a flat representation of the Element
   --  classification hierarchy defined by a set of the enumeration types in
   --  package Asis. All these types are defined as ordered types, so it would
   --  be consistent to have this flat representation to be an ordered type.
