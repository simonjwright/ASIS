...
package body Asis.Declarations is
   ...
   begin
      ...
      --  There is also a special case of expression function: it is rewritten
      --  as function declaration, and both original and rewritten nodes have
      --  aspect list. The problem with aspect lists in original node is that
      --  the Parent points to the original node that in turn does not have the
      --  Parent link. So in this case ASIS has to use the rewritten structure.
      --
      --  Note also, that there are cases when ASIS should NOT go into
      --  rewritten subtree. One of such cases is a library-level
      --  instantiation, but may be we have some more...

      if Has_Aspects (Arg_Node)
        and then
         not (Nkind (Arg_Node) = N_Expression_Function
             and then
              Is_Rewrite_Substitution (R_Node (Declaration))
             and then
              Nkind (R_Node (Declaration)) = N_Subprogram_Declaration
             and then
              Has_Aspects (R_Node (Declaration)))
      then
         ...
         if not (Is_Rewrite_Substitution (Arg_Node)
              and then
                 Nkind (Parent (Arg_Node)) = N_Compilation_Unit
              and then
                 Nkind (Orig_Node) in N_Generic_Instantiation)
         then
   ...
   begin
      ...
      if Nkind (Res_Node) = N_Null_Statement then
         --  This happens when an expression function is a completion of a
...
--  --|A2005 end
         ...
         when   A_Procedure_Body_Declaration
            ...
            if Nkind (Arg_Node) not in N_Package_Body |
...
--  --|A2012 start
         when An_Expression_Function_Declaration =>
            ...
            if Nkind (R_Node (Declaration)) /= N_Null_Statement then
               Arg_Node := R_Node (Declaration);
            end if;
...
--  --|A2012 end
   ...
   begin
      ...
      if Is_Part_Of_Instance (Defining_Name)
        and then
         Nkind (Arg_Node) /= N_Defining_Program_Unit_Name
        and then
         Special_Case (Defining_Name) /= Stand_Char_Literal
        and then
         Ekind (Arg_Node) = E_Subprogram_Body
      then
         EE := Enclosing_Element (Defining_Name);

         if Special_Case (EE) = Expanded_Subprogram_Instantiation then
      ...
      --  Name from the artificial renaming used to pass an actual subprogram
      --  into expanded instantiation

      Parent_N := Parent (Parent (Arg_Node));

      if Nkind (Parent_N) = N_Subprogram_Renaming_Declaration
        and then
         Present (Corresponding_Formal_Spec (Parent_N))
      then
         Arg_Node   := Corresponding_Formal_Spec (Parent_N);
         A4G.Mapping.Parent_Node := Parent (Arg_Node);
         Def_N_Kind := Asis_Internal_Element_Kind (Arg_Node);
      end if;
...
--  |A2005 end
   ...
   begin
      ...
      if Present (Sinfo.Expression (Parent (Arg_Node))) then
         ...
         while Present (Result_Node) loop
            ...
            then
               Result_Node :=
                 Parent_No_Normalization (Defining_Identifier (Result_Node));
         ...
         Result_Node := Parent_No_Normalization (Full_View (Arg_Node));
   ...
   begin
      ...
      Def_Kind := Int_Kind (Object_Declaration_View (Declaration));

      if Def_Kind in Internal_Access_Definition_Kinds then
         Raise_ASIS_Inappropriate_Element
           (Diagnosis  => Package_Name & "Declaration_Subtype_Mark",
            Wrong_Kind => Arg_Kind);
      end if;
   ...
   begin
      ...
      while No (Parent (Result_Node)) loop
   ...
   begin
      ...
      while No (Parent (Result_Node)) loop
         if Is_Itype (Result_Node) then
            ...
            if Etype (Result_Node) /= Result_Node
              and then
               Associated_Node_For_Itype (Result_Node) = Arg_Node
            then
               Result_Node := Etype (Result_Node);
            else
               Result_Node := Associated_Node_For_Itype (Result_Node);
...
--      Result_Node := Parent (Entity (Result_Node));
   ...
   is
      ...
      --
      --  These two arrays should be used for derived types only, so we need
      --  the following flag

      Is_Derived_Type : Boolean := False;

      Iterate_Through_Normalized_Declarations : Boolean := False;
      --  If we have an object declaration that defines more than one object,
      --  in the tree it is normalized into a chain of object declarations
      --  each defining a single object, and each of these declarations has
      --  a chain of representation items that are applied only to the object
      --  it defines. So to get the full set of representation clauses applied
      --  to the original multi-dimensional declaration we have to put together
      --  all the  representation clauses associated with a set of artificial
      --  normalized declarations

      First_Decl_Sloc : Source_Ptr := No_Location;
   ...
   begin
      ...
      Is_Derived_Type := Arg_Kind = A_Private_Extension_Declaration or else
        (Arg_Kind in An_Ordinary_Type_Declaration | A_Formal_Type_Declaration
        and then
         Int_Kind (Type_Declaration_View (Declaration)) in
      ...
      if Arg_Node_Kind  = N_Subprogram_Declaration          or else
         Arg_Node_Kind  = N_Expression_Function             or else
      ...
      then
         ...
         if Arg_Kind in A_Variable_Declaration | A_Constant_Declaration then
            First_Decl_Sloc                         := Sloc (Arg_Node);
            Iterate_Through_Normalized_Declarations := True;
         end if;
      end if;
      ...
      Through_Nodmalized_Declarations : loop
         while Present (Next_Rep_Item_Node) loop
            ...
            case Rep_Node_Kind is
               ...
               when N_Attribute_Definition_Clause =>
                  ...
                  if not From_At_Mod (Next_Rep_Item_Node)
                      and then
                        not (Is_Derived_Type
                          and then
                             not Actual_Rep_Attr (Chars (Next_Rep_Item_Node)))
                  then
               ...
               when others =>
                  --  This query is only about representation clauses,
                  --  not pragmas
                  null;
            end case;
         ...
         if Iterate_Through_Normalized_Declarations then
            Entity_Node := Next_Entity (Entity_Node);

            if No (Entity_Node) then
               Iterate_Through_Normalized_Declarations := False;
            else
               Iterate_Through_Normalized_Declarations :=
                  Nkind (Parent (Entity_Node)) = Arg_Node_Kind
                    and then Sloc (Parent (Entity_Node)) = First_Decl_Sloc;
            end if;
         end if;

         exit Through_Nodmalized_Declarations
           when not Iterate_Through_Normalized_Declarations;

         First_Rep_Item_Node := First_Rep_Item (Entity_Node);
         Next_Rep_Item_Node  := First_Rep_Item_Node;
      end loop Through_Nodmalized_Declarations;
...
--  |A2012 end
      ...
      if Nkind (R_Node (Declaration)) = N_Null_Statement then
         --  This happens when an expression function is a completion of a
         --  generic function declaration, the original tree structure does
         --  not have any semantic decoration
         Arg_Node := Next (R_Node (Declaration));

         if Nkind (Arg_Node) = N_Subprogram_Declaration then
            Arg_Node := Parent (Parent (Corresponding_Body (Arg_Node)));
         end if;
...
------------------------------------------------------------------------------
   function Is_Name_Repeated
     (Declaration : Asis.Declaration)
      return       Boolean
   is
      Arg       : constant Asis.Element           := Declaration;
   ...
   begin
      ...
      if not (Arg_Kind = A_Package_Declaration          or else
              ...
              Arg_Kind = An_Entry_Body_Declaration)
       or else
         Is_Part_Of_Implicit (Declaration)
       or else
      ...
      then
         ...
         if Is_Identical (Declaration, Unit_Declaration (Argument_Unit)) then
            ...
            then
               ...
               if Nkind (Arg_Node) = N_Null_Statement
                  and then
                    Nkind (Original_Node (Arg_Node)) = N_Expression_Function
               then
                  --  An expression function is a completion of a (generic???)
                  --  function
                  Arg_Node := Next (Arg_Node);

                  pragma Assert (Nkind (Arg_Node) = N_Subprogram_Body
                              and then
                               Is_Rewrite_Substitution (Arg_Node)
                              and then
                               Nkind (Original_Node (Arg_Node)) =
                                 N_Subprogram_Body);
               end if;
...
------------------------------------------------------------------------------
   ...
   type Attempts_For_On_The_Fly_Mode is (First, Second);
   Attempt : Attempts_For_On_The_Fly_Mode := First;
   --  For compile-on-the-fly mode the Corresponding_Body does the following
   --  trick. If the result computed by the query is Nil_Element, it checks if
   --  the body in question is the expanded body of some library-level
   --  instantiation or a subcomponent thereof, and if it is, the query
   --  computes the source of this instantiation, compiles this source for the
   --  tree, adds this tree to the Context, and tries to compute the result
   --  again.
   ...
   is
      ...
      Argument_Unit     : Asis.Compilation_Unit;
      Result_Unit       : Asis.Compilation_Unit := Nil_Compilation_Unit;
      Result_Cont       : constant Context_Id  := Encl_Cont_Id (Declaration);

      Use_Original_Node : Boolean := True;

      --  ???
      Inst_Source  : String_Access;
   ...
   begin
      ...
      --  Another special case: a (generic) function completed by expression
      --  function, the corresponding body could be in the same unit.

      if Int_Kind (Declaration) = A_Generic_Function_Declaration then
         Result_Node := Corresponding_Body (R_Node (Declaration));
      ...
      if not Exists (Result_Unit) then
         if No (Result_Node) then
            ...
            --  This is the case when a function declaration is completed by
            --  expression function in the declaration of a (generic) package
            Result_Unit := Argument_Unit;
         end if;
      end if;

      Correct_Renaming_Expansion (Result_Node);
      ...
      if Special_Case (Arg_Element) in Expanded_Spec then
         ...
         --  If this is an instance of a generic function whose completion
         --  was an expression function, then the compiler turned it into a
         --  function body. We need to undo that here (turn it back into an
         --  expression function).

         if Nkind (Result_Node) = N_Subprogram_Body
           and then not (Comes_From_Source (Result_Node))
           and then Was_Expression_Function (Result_Node)
         then
            Result_Kind := An_Expression_Function_Declaration;
         end if;
      end if;
      ...
      if Present (Result_Node) then
         ...
         Result := Node_To_Element_New
                     (Node              => Result_Node,
                      Internal_Kind     => Result_Kind,
                      Spec_Case         => Result_Spec_Case,
                      Use_Original_Node => Use_Original_Node,
                      In_Unit           => Result_Unit);
      elsif Attempt = First
        and then
            Tree_Processing_Mode (Result_Cont) in
              On_The_Fly | Mixed
        and then
            Body_In_Library_Level_Instantiation (Declaration, Inst_Source)
      then
         --  Try to add the tree for the library-level instantiation and to
         --  recompute the result in the Context that contains this tree.
         Attempt := Second;

         if not Is_Regular_File (Inst_Source.all) then
            Write_Str ("Corresponding_Body: cannot locate source of " &
                       "library-level instantiation");

            Attempt := First;
            return Result;  --  Nil_Element
         ...
         Create_Tree (Source_File   => Inst_Source,
                      Context       => Result_Cont,
                      Is_Predefined => Is_Predefined_File_Name (Inst_Source),
                      Success       => Success);

         if not Success then
            Write_Str ("Corresponding_Body: cannot compile source of " &
                       "library-level instantiation");
         ...
         Tree_File_N := Tree_From_Source_Name (Inst_Source);
         Tree_File_D := Open_Read (Tree_File_N.all'Address, Binary);

         begin
            Set_Name_String (To_String (Tree_File_N));
            Tree_In_With_Version_Check (Tree_File_D, Result_Cont, Success);
         ...
         exception
            when Version_Mismatch | ASIS_Failed   =>
               Attempt := First;
               raise;
            ...
            when Ex : others =>
               ...
               --  we have a tree file which is version-compatible with ASIS,
         ...
         Set_Name_String (To_String (Tree_File_N));
         Set_Current_Cont (Result_Cont);
         Result_Tree := Allocate_Tree_Entry;
         Set_Current_Tree (Result_Tree);
         Register_Units;
         Scan_Units_New;

         Result := Corresponding_Body (Declaration);

         Free (Inst_Source);
         Free (Tree_File_N);

      elsif Attempt = Second then
         Attempt := First;
      end if;
...
------------------------------------------------------------------------------
   ...
   begin
      ...
      then
         ...
         if Nkind (Result_Node) = N_Formal_Type_Declaration then
            Result_Node :=
              Sinfo.Formal_Type_Definition (Original_Node (Result_Node));
         else
   ...
   begin
      ...
      if Is_Bool_Eq_Declaration (Declaration) then
         ...
         if Is_Tagged_Type_Eq (Declaration) then
            ...
            while Present (Res_Node)
                 and then
                  not (Nkind (Res_Node) = N_Defining_Operator_Symbol
                    and then
         ...
         if Is_Tagged_Type_Eq (Declaration)
           or else
            Is_Based_On_Same_Node (Declaration)
         then
            Result := Declaration;
            Set_Special_Case (Result, Not_A_Special_Case);
            Set_From_Implicit  (Result, Present (Node_Field_1 (Declaration)));
            Set_From_Inherited (Result, Present (Node_Field_1 (Declaration)));
