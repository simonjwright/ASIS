...
package body GNATtest.Skeleton.Source_Table is
   ...
   begin
      ...
      if Verbose then
         Trace (Me, "Current infix is " & Current_Source_Infix);
         Trace (Me, "Root of subtree is " & Subroot_Prj_Name);
         Trace (Me, "excluded sources are:");
         Increase_Indent (Me);
         SS_Cur := Excluded_Sources.First;
            while SS_Cur /= String_Set.No_Element loop
               Trace (Me, String_Set.Element (SS_Cur));
               Next (SS_Cur);
            end loop;
         Decrease_Indent (Me);
      end if;
   ...
   begin
      ...
      exception
         when Constraint_Error =>
            Report_Err ("gnattest: inconsistent state of sources detected");
            raise Fatal_Error;
      end;
